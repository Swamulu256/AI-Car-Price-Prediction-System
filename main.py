...
from regression import ALGORITHM
...
class CAR_PRICE_PREDICTION:
    ...
    def all_algo(self):
        try:
            ...
            logger.info(f'\n{self.y_train}')
            logger.info(f'\n{self.y_test}')
            ALGORITHM.model_training(self.training_data,self.testing_data,self.y_train,self.y_test)
        except Exception as e:
            error_type, error_msg, error_line = sys.exc_info()
            logger.info(f'Error in line no:{error_line.tb_lineno} due to:{error_msg}')
...
if __name__ == '__main__':
    try:
        obj=CAR_PRICE_PREDICTION('C:\\Users\\DELL\\Downloads\\AI-Car-Price-Prediction-System\\data (1).csv')
        obj.missing_values()
